  ...
  describe('sendEmail', () => {
    it('should send a transactional email successfully', async () => {
      ...
      expect(mockSend).toHaveBeenCalledWith({
        from: 'Open Kernel <noreply@openkernel.ai>',
    ...
    it('should send a marketing email with unsubscribe headers', async () => {
      ...
      expect(mockSend).toHaveBeenCalledWith({
        ...
        headers: {
          'List-Unsubscribe':
            '<https://test.openkernel.ai/unsubscribe?token=mock-token-123&email=test%40example.com>',
    ...
    it.concurrent('should use custom from address when provided', async () => {
      ...
      expect(mockSend).toHaveBeenCalledWith(
        expect.objectContaining({
          from: 'Open Kernel <custom@example.com>',
        })
      )
    })
